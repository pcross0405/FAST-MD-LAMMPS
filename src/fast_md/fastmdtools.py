...
def check_center(tm_pairs, added_atoms, lmp, n, min_nrg, iso_limit, new_id, min_style = 'sd'):
    ...
    for i, center in enumerate(current_centers):
        ...
        if np.abs(iso_atom_x[0]/xmax) < 0.0001 or 1 - np.abs(iso_atom_x[0]/xmax) < 0.0001:
            if np.abs(center[0]/xmax) < 0.0001 or 1 - np.abs(center[0]/xmax) <  0.0001:
                iso_atom_x[0] = center[0]
        
        if np.abs(iso_atom_y[0]/ymax) < 0.0001 or 1 - np.abs(iso_atom_y[0]/ymax) < 0.0001:
            if np.abs(center[1]/ymax) < 0.0001 or 1 - np.abs(center[1]/ymax) <  0.0001:
                iso_atom_y[0] = center[0]

        if np.abs(iso_atom_z[0]/zmax) < 0.0001 or 1 - np.abs(iso_atom_z[0]/zmax) < 0.0001:
            if np.abs(center[2]/zmax) < 0.0001 or 1 - np.abs(center[2]/zmax) <  0.0001:
                iso_atom_z[0] = center[0]
